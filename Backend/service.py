...
SMART_CONTRACT_HASH = "3fccfee0599aa9fed495a281488de6f5a59e0231"
...
def sc_notify(event):
    logger.info("Storage event : %s", event)
    ...
    bid, bbalance = event.event_payload[0]
    logger.info("- payload: %s %s" % (int.from_bytes(bid), int.from_bytes(bbalance)))

    BALANCES[bid] = max(bbalance, BALANCES[bid])
...
def home(request):
    return {
        "balance": BALANCES[USER_ID] if USER_ID in BALANCES else 0
    }


@app.route('/perk/<perk_id>/<perk_amount>')
@catch_exceptions
@json_response
def echo_msg(request, perk_id, perk_amount):
    return {
        "perk_id": perk_id,
